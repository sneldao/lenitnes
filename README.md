...
5. **Commit** — open a tracked position in the recommended direction, long or short. Longs on BTC/ETH swap on-chain via PancakeSwap; **shorts and L1 assets (ZEC/SOL/SUI/ARB) now execute live as Hyperliquid perps via [Propr](https://www.propr.xyz/developers)** — the first venue with a short primitive. All gated behind a double kill switch (`TRADING_ENABLED` + `PROPR_ENABLED`, both default off); paper mode is the safe fallback. Every fill is notarized on Hedera HCS, broadcast to Telegram.
