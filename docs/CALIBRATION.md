...
| `POSITION_TAKE_PROFIT_BPS`     | `1500`     | services/treasury/risk.ts          | Conviction-scaled base          |
| `POSITION_STOP_LOSS_BPS`       | `700`      | services/treasury/risk.ts          | Conviction-adaptive base        |
...
| `PROPR_MIN_NOTIONAL_USD`       | `20`       | services/venues/propr/index.ts     | Conviction-scaled floor         |
| `PROPR_MAX_NOTIONAL_USD`       | `500`      | services/venues/propr/index.ts     | Conviction-scaled ceiling       |
...
| 2026-07-11 | Telegram verdict digest + narrative signal format                                       | Verdict posts: repo/date/thesis/cohort stats; batch digest for same-asset bursts; plain-language price vs SHORT/LONG; signal posts add repo+detector+deep links — see RUNBOOK § Public Telegram format                                                                                                                                                                                              |
| 2026-07-25 | Propr Hyperliquid perp venue: live shorts + L1 assets (ZEC/SOL/SUI/ARB)                 | First venue with a short primitive; gated behind TRADING_ENABLED + PROPR_ENABLED; 3-tier account discovery; conviction-scaled SL+TP; reduceOnly closes; HCS notarization; notional clamped $20-$500                                                                                                                                                                                                 |
| 2026-07-26 | Conviction-scaled position sizing (Propr notional)                                      | Notional now linear from $20 (conviction 70) to $500 (conviction 100) instead of flat $500. Higher conviction → bigger position. `computeConvictionNotional` in venues/propr/index.ts                                                                                                                                                                                                               |
| 2026-07-26 | Adaptive stop loss (conviction-scaled SL)                                               | SL widens with conviction: 5% at 70 → 9% at 100. Uncertain theses cut fast, strong theses get room to breathe. `computeTpSlLevels` in treasury/risk.ts now scales SL bps by conviction (was fixed 700)                                                                                                                                                                                              |
| 2026-07-26 | Thesis synthesis: aggregate un-triggered commits into tradeable theses                  | Every 2h, fetches settled commits from all monitors, excludes signaled ones, and when pool ≥4 across ≥2 repos (or ≥6 single repo), asks agent if collective pattern forms a thesis. Full commit SHAs in evidence. `runThesisSynthesis` in services/agent/thesis-synthesis.ts                                                                                                                        |
| 2026-07-26 | Velocity anomaly detection                                                              | Tracks commit rate per repo over 30d baseline, flags ±2σ deviations in 7d window. Spikes = emergency response; drops = maintainer departure. `detectVelocityAnomalies` in services/detectors/velocity-anomaly.ts                                                                                                                                                                                    |
| 2026-07-26 | PR activity monitoring                                                                  | Scores open PRs by title keywords (breaking/consensus/security), comment count, size, review activity, labels. Threshold ≥60. `detectHighImpactPRs` in services/detectors/pr-activity.ts                                                                                                                                                                                                            |
| 2026-07-26 | Proactive signals orchestrator                                                          | Runs velocity + PR detectors every 2h (odd hours), creates signals, scores via agent, trades if conviction ≥70. `runProactiveScan` in services/agent/proactive-signals.ts                                                                                                                                                                                                                           |
| 2026-07-28 | API spend guard (Redis-shared daily budgets, fail-closed)                               | TinyFish burned credits in a zero-credit crash loop and CoinGecko hit ~300×429/day, which silently declined every Propr fill ("no mark price"). All providers now route through `services/spend-guard.ts`: `API_BUDGET_COINGECKO_PER_DAY=300`, `API_BUDGET_CMC_PER_DAY=400`, `API_BUDGET_TINYFISH_PER_DAY=0` (0 = disabled). Budget errors throw before HTTP and consumers degrade to defer/decline |
| 2026-07-28 | Spot price hub (one batched CMC call for all assets)                                    | Every TP/SL tick, Propr sizing call, entry-price write and matured-outcome snapshot used to mint its own per-asset CoinGecko range call. Now `SPOT_PRICE_REFRESH_SECONDS=600` warms `spot:usd:<id>` in Redis for the whole watchlist with 1 credit; `PRICE_FRESH_SLOP_SECONDS=720` routes near-now lookups to the hub (`services/data-providers/spot-prices.ts`)                                    |
| 2026-07-28 | TinyFish retired (TINYFISH_ENABLED=false) + virtual monitors bypass the scrape pipeline | github-direct enrichment (free GitHub API) already carried the real gate for every repo monitor. `narrative:`/`synthesis:`/`proactive:` pseudo-URL monitors were reaching the HTTP scraper, crashing it with "unknown scheme" on ~5.8k DLQ'd jobs. Both classes never enter the scrape tiers now                                                                                                    |
| 2026-07-28 | CoinGecko 15-min cache-key bucketing + shared post-429 cooldown + CMC parser fix        | Bucketed keys make repeated lookups for the same signal hit cache; the cooldown stops retry loops against an account-wide quota. CMC global metrics parser now reads `quote.USD` (was rendering "Market Cap: $0" in every agent context); system prefers the flat-rate CMC key over pay-per-call x402                                                                                               |
| 2026-07-28 | Keyless price fallback chain (hyperliquid → kraken → defillama → coingecko)             | Quota debt no longer blocks outcome windows, backfills or tier sweeps: span-aware chain of keyless oracles, per-source circuit breakers, unified cache population. First-measured effect: 90d sweep graded bitcoin/bitcoin + ethereum/go-ethereum as A-tier (70 floor) within 2 min of boot; the 45s /portfolio backfill hang became 0.46s. Kill-switch `PRICE_FALLBACKS_ENABLED=false`             |
